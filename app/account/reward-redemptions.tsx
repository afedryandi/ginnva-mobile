...
// Mirror app/partner/redemptions.tsx — Partner sudah punya layar riwayat
// penukaran reward, tapi Customer sebelumnya TIDAK, walau endpoint
// /api/customer/redemptions sudah ada & berfungsi (PointController::
// redemptions()) sejak lama. Setelah tukar reward di app/rewards.tsx,
// customer sebelumnya tidak punya cara sama sekali untuk cek status
// klaimnya (masih diproses/sudah dikirim/dibatalkan) selain mengandalkan
// ingatan dari Alert sekali saat redeem. Dibangun saat audit modul
// Marketing > Klaim Reward.
...
export default function RewardRedemptionsScreen() {
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    setError(null);
    try {
      const res = await apiFetch<{ data: Redemption[] }>('/api/customer/redemptions');
      setItems(res.data ?? []);
    } catch (err) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle} numberOfLines={1}>Riwayat Tukar Reward</Text>
        <View style={styles.sideButton} />
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>Reward yang kamu tukar dengan poin akan muncul di sini.</Text>
            </View>
          }
        />
      )}
